...
cn_prefix="/C=ES/ST=MA/L=Madrid/O=MongoDB Spain"
ou_member="MyServers"
ou_client="MyClients"
mongodb_server_hosts=( "server1" "server2" "server3" )
mongodb_client_hosts=( "ruben" "client2" )
mongodb_port=27017
...
db.getSiblingDB("admin").createUser({user: "ruben", pwd: "1234", roles: ["dbAdminAnyDatabase"]});
EOF
cat setup_auth.js
...
mongo --ssl --sslPEMKeyFile ${mongodb_client_hosts[0]}.pem --sslCAFile root-ca.pem --sslAllowInvalidHostnames --shell do_login.js
